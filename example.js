...
import { response, cookies, secrets, Stopwatch, WebsocketResponse } from './src/index.js'

router.get('/json', async (req) => {
    return response.json({'Hello': 'world!'})
})

router.get('/websocket', async (req) => {
    var socket = new WebsocketResponse()

    socket.on('message', (msg) => {
        socket.send(msg)
        setTimeout(() => {
            socket.send({ data: msg, currentTime: new Date().toLocaleString() })
        }, 1000)
    })
